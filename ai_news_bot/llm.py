...
SYSTEM_PROMPT = """你是严谨的中文科技周报编辑。
请根据用户提供的候选新闻生成中文周报，英文标题和摘要必须翻译或改写为中文。优先挑选本周最重要、影响面最大的条目，避免流水账堆砌。
硬性规则：
...
4. 周报头部使用 "# 📡 Furina · 每周科技/AI周报"（Markdown 一级标题）。
...
13. GitHub 两节禁止写“（来源：…）”尾巴；条目格式必须是：`1. [owner/repo](https://github.com/owner/repo) 中文一句话`。
14. 禁止空话：不要写「具体功能待探索」「值得持续关注」等无信息表述；摘要不足就只保留仓库链接与候选摘要中的事实。
"""
...
def build_chat_payload(
    ...
    user_prompt = f"""请把以下候选新闻整理成周报。
周报周期：{date_label}
必须原样使用这个周期/日期，不要自行推断、改写或替换日期。

候选新闻：
{_format_items(items)}

请直接输出周报正文，不要解释生成过程。内容要比快讯更耐读，偏「本周要点回顾」，仍然适合微信消息阅读。
